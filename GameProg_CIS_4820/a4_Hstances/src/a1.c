...
int amI_Falling();
void Jump_Routine(float timePassed);
...
static double projectile_Angle=28;
static double projectile_Velocity=28;
...
void draw2D() 
{
   ...
   /* your code goes here */
    ...
    if(displayMap==1)
    {
      ...
      DrawBorder_M1(0,0,mapW, mapH,200);
    ...
    else if (displayMap==2)
    {
      ...
      set2Dcolour(color);
      //Same Params as Map Backdrop
      DrawBorder_M2(0,0,mapW, mapH,200);
...
float magnitude_Jump=7;

float cur_mag_Jump=0;


float jump_Delay=5;

float time_SinceJump=5;

int amI_Falling()
{
  //Get getOldViewPosition
  float oldY;
  float oldX;
  float oldZ;
...
//The Jump occurs in less time than 5 seconds therefore the player can't double/triple jump here
void Jump_Routine(float timePassed)
{
  time_SinceJump+=timePassed;

  //Check if we are requested to Jump?
  if(space==TRUE)
  {
    space=FALSE;
    //printf("SPACE!\n");
    if(time_SinceJump>=jump_Delay)
    {
      //Reset Jump Clock
      time_SinceJump=0;
      cur_mag_Jump=magnitude_Jump;
    }
  ...
  getViewPosition(&curX, &curY, &curZ);  
  setViewPosition(curX, curY-(cur_mag_Jump*timePassed), curZ);
  //Next time this is called cur_mag_Jump will have a smaller value

  //printf("CURJUMP= %f \n", cur_mag_Jump);
  cur_mag_Jump-= (2)*timePassed;
  if(cur_mag_Jump<0)
    cur_mag_Jump=0;
}
