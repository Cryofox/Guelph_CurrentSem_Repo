...
void CreateSkyClouds()
    ...
    //First Value must be Center
      ...
      for(int i=1;i<cloud->cloudLength;i++)
      {
         ...
         for(int j=0;j<i;j++)
         {
            ...
            int xPos= (int)cloud->cloud_voxels[j].pos_X;
            int zPos= (int)cloud->cloud_voxels[j].pos_Z;
   ...
   int isDuplicate(Cloud* cloud, int xOffset, int zOffset)
   {
      ...
      {
         if((int)cloud->cloud_voxels[i].pos_X == xOffset && (int)cloud->cloud_voxels[i].pos_Z == zOffset)   
         {
         return 1;
         }
      }
